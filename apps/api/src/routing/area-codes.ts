/**
 * US area code → state, for geo routing. Callers can port numbers between states,
 * so this is "where the number is from", which is what the industry routes on.
 */
...
for (const [state, codes] of Object.entries(BY_STATE)) {
  for (const code of codes.split(' ')) AREA_TO_STATE.set(code, state);
}

/** "+14155550123" → "CA". null for toll-free, non-US or unknown. */
export function stateForNumber(e164: string): string | null {
  const m = e164.match(/^\+1(\d{3})\d{7}$/);
  return m ? AREA_TO_STATE.get(m[1]) ?? null : null;
}
