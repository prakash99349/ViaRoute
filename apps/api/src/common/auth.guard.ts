...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    const current = await prisma.user.findUnique({
      where: { id: user.sub },
      select: { tenantId: true, role: true, tokenVersion: true, passwordHash: true, publisherId: true, buyerId: true, timezone: true, disabledAt: true, agent: { select: { targetId: true } } },
    });
    ...
    user.agentTargetId = current.agent?.targetId ?? null;
    ...
    // Agents only reach what explicitly allows them (their softphone and their calls), plus their own login.
    if (user.role === Role.AGENT && !roles?.includes(Role.AGENT) && !req.path.startsWith('/auth') && !(req.method === 'GET' && req.path === '/tenant')) {
