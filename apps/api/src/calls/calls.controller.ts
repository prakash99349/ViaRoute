...
export const CDR_COLUMNS: Column[] = [
  ...
  { key: 'ivr_path', label: 'IVR choices', default: false, value: (c) => c.ivrPath ?? '' },
  { key: 'ivr_data', label: 'IVR entries', default: false, value: (c) => Object.entries((c.ivrData ?? {}) as Record<string, string>).map(([k, v]) => `${k}=${v}`).join('; ') },
  { key: 'spam_score', label: 'Spam score', default: false, roles: STAFF, value: (c) => c.spamScore ?? '' },
  { key: 'attestation', label: 'STIR/SHAKEN', default: false, roles: STAFF, value: (c) => c.attestation ?? '' },
